...
function WeatherApp() {
  ...
  return (
    <div className='bg-gradient-to-r from-yellow-200 to-yellow-300 w-full h-screen  flex flex-col justify-center items-center '>
      <h1 className='font-black-ops text-[30px] text-white'>WEATHER APP</h1>
      <div className='text-lg flex font-black-opsjustify-between w-[20rem] md:w-[25rem] px-1 md:px-2'> 
        <p className='ml-5 text-white font-bold'>{formatDate(currentDateTime)}</p>
        <p className='ml-5 text-white font-bold'>{currentDay()}</p>
        <p className='ml-4 text-white font-bold'>{formatTime(currentDateTime)}</p>
      </div>

      {values.city &&
        <div className='w-[20rem] md:w-[25rem] h-[30rem] bg-yellow-400 rounded-lg'>
